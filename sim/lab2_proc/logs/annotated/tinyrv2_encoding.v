//      // verilator_coverage annotation
        ...
        module lab2_proc_tinyrv2_encoding_InstTasks();
          ...
          // line tracing
          //----------------------------------------------------------------------
 000300   function [11:0] imm_i( input [`TINYRV2_INST_NBITS-1:0] inst );
 000300   begin
            // I-type immediate
 000300     imm_i = { inst[31], inst[30:25], inst[24:21], inst[20] };
          end
          endfunction
...
%000000     imm_shamt = { inst[24:21], inst[20] };
 ...
 000100   function [11:0] imm_s( input [`TINYRV2_INST_NBITS-1:0] inst );
 000100   begin
            // S-type immediate
 000100     imm_s = { inst[31], inst[30:25], inst[11:8], inst[7] };
          end
          endfunction
        
 000025   function [12:0] imm_b( input [`TINYRV2_INST_NBITS-1:0] inst );
 000025   begin
            // B-type immediate
 000025     imm_b = { inst[31], inst[7], inst[30:25], inst[11:8], 1'b0 };
          end
          endfunction
...
%000000   logic [`TINYRV2_INST_FUNCT7_NBITS-1:0] funct;
 ...
 000544   function [25*8-1:0] disasm( input [`TINYRV2_INST_NBITS-1:0] inst );
 000544   begin
        
            // Unpack the fields
        
 000544     rs1      = inst[`TINYRV2_INST_RS1];
 000544     rs2      = inst[`TINYRV2_INST_RS2];
 000544     rd       = inst[`TINYRV2_INST_RD];
 000544     csr      = inst[`TINYRV2_INST_CSR];
            // xcel
 000544     funct    = inst[`TINYRV2_INST_FUNCT7];
        
            // Create fixed-width register specifiers
        
%000000     if ( rs1 <= 9 )
 000544       $sformat( rs1_str, "x0%0d", rs1 );
            else
%000000       $sformat( rs1_str, "x%d",  rs1 );
        
 000250     if ( rs2 <= 9 )
 000294       $sformat( rs2_str, "x0%0d", rs2 );
            else
 000250       $sformat( rs2_str, "x%d",  rs2 );
        
 000150     if ( rd <= 9 )
 000394       $sformat( rd_str, "x0%0d", rd );
            else
 000150       $sformat( rd_str, "x%d",  rd );
        
 000001     if ( csr == `TINYRV2_CPR_PROC2MNGR )
 000001       $sformat( csr_str, "proc2mngr" );
 000005     else if ( csr == `TINYRV2_CPR_MNGR2PROC )
 000005       $sformat( csr_str, "mngr2proc" );
...
%000000       $sformat( csr_str, "numcores " );
 000538     else if ( csr == `TINYRV2_CPR_STATS_EN )
%000000       $sformat( csr_str, "stats_en " );
            else
 000538       $sformat( csr_str, "    0x%x", csr );
        
 000544     $sformat( funct_str, "%x", funct[1:0]);
        
            // Actual disassembly
        
 000544     casez ( inst )
 000005       `TINYRV2_INST_CSRR  : $sformat( disasm, "csrr   %s, %s  ",        rd_str,  csr_str );
 000001       `TINYRV2_INST_CSRW  : $sformat( disasm, "csrw   %s, %s  ",        csr_str, rs1_str );
 000006       `TINYRV2_INST_NOP   : $sformat( disasm, "nop                    " );
 000006       `TINYRV2_ZERO       : $sformat( disasm, "                       " );
        
 000101       `TINYRV2_INST_ADD   : $sformat( disasm, "add    %s, %s, %s   ",   rd_str,  rs1_str, rs2_str );
...
%000000       `TINYRV2_INST_MUL   : $sformat( disasm, "mul    %s, %s, %s   ",   rd_str,  rs1_str, rs2_str );
 ...
 000100       `TINYRV2_INST_ADDI  : $sformat( disasm, "addi   %s, %s, 0x%x ",   rd_str,  rs1_str, imm_i(inst) );
...
%000000       `TINYRV2_INST_AUIPC : $sformat( disasm, "auipc  %s, 0x%x    ",    rd_str,  imm_u_sh12(inst));
 ...
 000200       `TINYRV2_INST_LW    : $sformat( disasm, "lw     %s, 0x%x(%s) ",   rd_str,  imm_i(inst), rs1_str );
 000100       `TINYRV2_INST_SW    : $sformat( disasm, "sw     %s, 0x%x(%s) ",   rs2_str, imm_s(inst), rs1_str );
...
%000000       `TINYRV2_INST_BEQ   : $sformat( disasm, "beq    %s, %s, 0x%x",    rs1_str, rs2_str, imm_b(inst) );
 000025       `TINYRV2_INST_BNE   : $sformat( disasm, "bne    %s, %s, 0x%x",    rs1_str, rs2_str, imm_b(inst) );
...
%000000       default             : $sformat( disasm, "illegal inst           " );
 ...
 001627   function [4*8-1:0] disasm_tiny( input [`TINYRV2_INST_NBITS-1:0] inst );
 001627   begin
        
 001627     casez ( inst )
 000015       `TINYRV2_INST_CSRR  : disasm_tiny = "csrr";
 000003       `TINYRV2_INST_CSRW  : disasm_tiny = "csrw";
 000018       `TINYRV2_INST_NOP   : disasm_tiny = "nop ";
        
 000303       `TINYRV2_INST_ADD   : disasm_tiny = "add ";
...
%000000       `TINYRV2_INST_MUL   : disasm_tiny = "mul ";
 ...
 000300       `TINYRV2_INST_ADDI  : disasm_tiny = "addi";
...
%000000       `TINYRV2_INST_CUST0 : disasm_tiny = "cus0";
 ...
 000008   input  [`TINYRV2_INST_NBITS-1:0]        inst,
        
          // Packed fields
        
 000020   output [`TINYRV2_INST_OPCODE_NBITS-1:0] opcode,
 000350   output [`TINYRV2_INST_RD_NBITS-1:0]     rd,
 000008   output [`TINYRV2_INST_RS1_NBITS-1:0]    rs1,
 000140   output [`TINYRV2_INST_RS2_NBITS-1:0]    rs2,
 000270   output [`TINYRV2_INST_FUNCT3_NBITS-1:0] funct3,
%000000   output [`TINYRV2_INST_FUNCT7_NBITS-1:0] funct7,
 000140   output [`TINYRV2_INST_CSR_NBITS-1:0]    csr
        );
