//      // verilator_coverage annotation
 ...
 012630   input  logic enq_val,        // Enqueue data is valid
 000098   output logic enq_rdy,        // Ready for producer to do an enqueue
        
 012578   output logic deq_val,        // Dequeue data is valid
 008103   input  logic deq_rdy,        // Consumer is ready to do a dequeue
        
 010004   output logic write_en,       // Write en signal to wire up to storage element
 009993   output logic bypass_mux_sel, // Used to control bypass mux for bypass queues
 009993   output logic num_free_entries // Either zero or one
        );
        
          // Status register
        
 009993   logic full;
 010004   logic full_next;
        
 037572   always_ff @(posedge clk) begin
 037572     full <= reset ? 1'b0 : full_next;
          end
...
%000000   logic  do_bypass;
 ...
 000088   input  logic                   write_en,
 000088   input  logic                   bypass_mux_sel,
 ...
 000026   input  logic                    clk, reset,
        
 005920   input  logic                    enq_val,        // Enqueue data is valid
%000000   output logic                    enq_rdy,        // Ready for producer to enqueue
        
 004458   output logic                    deq_val,        // Dequeue data is valid
 005587   input  logic                    deq_rdy,        // Consumer is ready to dequeue
        
 003612   output logic                    write_en,       // Wen to wire to regfile
 001873   output logic [c_addr_nbits-1:0] write_addr,     // Waddr to wire to regfile
 001865   output logic [c_addr_nbits-1:0] read_addr,      // Raddr to wire to regfile
 003598   output logic                    bypass_mux_sel, // Control mux for bypass queues
 003598   output logic [c_addr_nbits:0]   num_free_entries // Num of free entries in queue
        );
        
          // Enqueue and dequeue pointers
        
 001873   logic [c_addr_nbits-1:0] enq_ptr;
 001879   logic [c_addr_nbits-1:0] enq_ptr_next;
 ...
 001865   logic [c_addr_nbits-1:0] deq_ptr;
 001873   logic [c_addr_nbits-1:0] deq_ptr_next;
...
%000000   logic  do_pipe;
 ...
 001865   logic [c_addr_nbits-1:0] deq_ptr_plus1;
          assign deq_ptr_plus1 = deq_ptr + 1'b1;
        
          /* verilator lint_off WIDTH */
        
 001865   logic [c_addr_nbits-1:0] deq_ptr_inc;
          assign deq_ptr_inc = (deq_ptr_plus1 == p_num_msgs) ? {c_addr_nbits{1'b0}} : deq_ptr_plus1;
        
 001873   logic [c_addr_nbits-1:0] enq_ptr_plus1;
          assign enq_ptr_plus1 = enq_ptr + 1'b1;
        
 001873   logic [c_addr_nbits-1:0] enq_ptr_inc;
 ...
 003598   input  logic                    bypass_mux_sel,
...
%000000   logic [p_msg_nbits-1:0] read_data;
 ...
 000090   input  logic                   enq_val,
 000098   output logic                   enq_rdy,
 000682   input  logic [p_msg_nbits-1:0] enq_msg,
        
 000066   output logic                   deq_val,
 001658   input  logic                   deq_rdy,
 000575   output logic [p_msg_nbits-1:0] deq_msg,
        
 000088   output logic [c_addr_nbits:0]  num_free_entries
        );
